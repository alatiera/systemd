...
static int parse_argv(
        ...
        for (int i = 0; i < argc; i++) {
                ...
                } else if (startswith(argv[i], "suspend="))
                        pam_syslog(handle, LOG_NOTICE, "Parameter 'suspend=' is ignored since systemd v260. Please use 'can-secure-lock' on pam_systemd instead.");
                else
...
static int acquire_home(
        ...
        for (;;) {
                ...
                r = sd_bus_message_append(m, "b", true); /* please_suspend is ignored as of v257... */
                if (r < 0)
                        return pam_bus_log_create_error(handle, r);
