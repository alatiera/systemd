...
int user_new(Manager *m, UserRecord *ur, User **ret) {
        ...
        *u = (User) {
                ...
                .secure_locked = streq_ptr(ur->state, "locked"),
        };
...
User *user_free(User *u) {
        ...
        sd_varlink_unref(u->pending_secure_lock_call);
...
int user_finalize(User *u) {
        ...
        if (u->n_pending_secure_locks > 0) { /* We have pending SecureLock() calls */
                ...
                u->pending_secure_lock_call = sd_varlink_close_unref(u->pending_secure_lock_call);
...
static int user_on_secure_lock_reply(
        ...
        if (error_id) {
                if (STR_IN_SET(error_id,
                        ...
                        sd_bus_error_set_errno(&error, sd_varlink_error_to_errno(error_id, parameters));
        }

        return user_finish_secure_lock(u, sd_bus_error_is_set(&error) ? &error : NULL);
}
...
static int user_trigger_secure_lock(User *u) {
        ...
        r = sd_varlink_attach_event(link, u->manager->event, SD_EVENT_PRIORITY_NORMAL);
        if (r < 0)
                return log_error_errno(r, "%s: Failed to attach to event loop: %m", path);

        (void) sd_varlink_set_userdata(link, u);
        (void) sd_varlink_set_description(link, path);

        r = sd_varlink_bind_reply(link, user_on_secure_lock_reply);
        if (r < 0)
                return log_error_errno(r, "%s: Failed to attach reply callback: %m", path);

        r = sd_varlink_invokeb(link,
                            "io.systemd.SecureLockBackend.Activate",
                            SD_JSON_BUILD_OBJECT(
                                SD_JSON_BUILD_PAIR_STRING("service", u->user_record->service),
                                SD_JSON_BUILD_PAIR_STRING("userName", u->user_record->user_name)));
        if (r < 0)
